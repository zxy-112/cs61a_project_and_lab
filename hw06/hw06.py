    ...
    def vend(self):
        if self.stock == 0:
            ...
            if current_balance == self.price:
                ...
                return 'Here is your {} and ${} change.'.format(self.name, current_balance - self.price)
...
def is_bst(t):
    ...
    # else:
    #     if len(t.branches) == 2:
    #         if not(t.branches[0].label <= t.label and t.branches[1].label > t.label):
    #             return False
    ...
    if t.is_leaf():
        ...
        if len(t.branches) == 2:
            if not (t.label >= biggest(t.branches[0]).label and t.label < smallest(t.branches[1]).label):
                return False
            return all([is_bst(branch) for branch in t.branches])
        if len(t.branches) == 1:
            if not (t.label >= biggest(t.branches[0]).label or t.label < smallest(t.branches[0]).label):
                return False
...
def store_digits(n):
    ...
    def helper(num, s):
        if num == 0:
            return s
        return helper(num // 10, Link(num % 10, s))
...
def remove_all(link , value):
    ...
    if link.rest is Link.empty:
        return
    tmp = link.rest
    while tmp is not Link.empty and tmp.first == value:
        tmp = tmp.rest
    link.rest = tmp
    if tmp is not Link.empty:
        remove_all(tmp, value)
