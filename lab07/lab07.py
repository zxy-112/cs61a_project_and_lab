...
def trade(first, second):
    ...
    while not equal_prefix() and not (m == len(first) and n == len(second)):
        if sum(first[:m]) < sum(second[:n]) and not m == len(first):
            m += 1
        else:
            n += 1
...
def make_glookup(class_assignments):
    ...
    acquired = 0
    total = 0
    def glookup(key, grade):
        nonlocal acquired, total
    ...
    *   *        *       *
    ...
    if n <= 2:
        ...
        for k in range(1, n):
            if k not in res:
                res[k] = num_trees(k)
        return sum([res[k] * res[n-k] for k in range(1, n)])
...
def make_advanced_counter_maker():
    ...
    1
    """
    global_res = {'global-count': 0, 'global-reset': 0}
    def somefunc():
        local_res = {'count': 0, 'reset': 0}
        def anotherfunc(word):
            nonlocal global_res, local_res
            "*** YOUR CODE HERE ***"
            # as many lines as you want
            if word in global_res:
                if word == 'global-count':
                    global_res[word] += 1
                    return global_res[word]
                else:
                    global_res['global-count'] = 0
            else:
                if word == 'count':
                    local_res[word] += 1
                    return local_res[word]
                else:
                    local_res['count'] = 0
        return anotherfunc
    return somefunc
