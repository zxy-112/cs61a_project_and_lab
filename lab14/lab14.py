def prune_min(t):
    ...
    if not(t.is_leaf()):
        if len(t.branches) == 1:
            ...
            t.branches = [t.branches[min(range(len(t.branches)), key=lambda x: t.branches[x].label)]]
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            corrections, cost = helper_align(skeleton_idx+1, code_idx+1)
            possibilities.append((skel_char + corrections, cost))
        # Insert
        corrections, cost = helper_align(skeleton_idx, code_idx+1)
        corrections, cost = '+[' + code[code_idx] + ']' + corrections, cost + 1
        possibilities.append((corrections, cost))
        # Delete
        corrections, cost = helper_align(skeleton_idx + 1, code_idx)
        corrections, cost = '-[' + skeleton[skeleton_idx] + ']' + corrections, cost + 1
        possibilities.append((corrections, cost))
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    total = sum(s)
    is_okay = lambda x: abs(total - 2 * x) <= d
    length = len(s)
    count = 0
    def helper(cum, index):
        nonlocal count
        if index == length:
            if is_okay(cum):
                count += 1
        else:
            helper(cum+s[index], index+1)
            helper(cum, index+1)
    helper(0, 0)
    return count // 2
...
def insert(link, value, index):
    ...
    if index == 0 and link is not Link.empty:
        ...
        link.first = value
        link.rest = new
    elif index > 0 and link is not Link.empty:
